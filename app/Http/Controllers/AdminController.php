...
class AdminController extends Controller {
    ...
    public function settings_store(Request $request) {
        ...
        if (!file_exists($config)) {
            ...
            $data = "<?php 
				...
				return array(
					...
					'sos_number' => '911',
					'contact_number' => '',
					'contact_email' => 'admin@mobilidadeurbana.com.br',
					'environment' => '',
					'ios_push_password' => '',
					'android_push_key' => '',
        ...
        if (!$request->has('ride_toll')) {
            $search_text = "'ride_toll' => '1'";
            $value_text = "'ride_toll' => '0'";
            $change_content = str_replace($search_text, $value_text, $change_content);
        }
        ...
        if ($request->hasFile('site_email_logo')) {
            ...
            $search_text = "'site_email_logo' => '" . config('constants.site_email_logo') . "'";
    ...
    public function settings_payment_store(Request $request) {
        ...
        if (!$request->has('daily_target')) {
            ...
            //TODO ALLAN - Alterações débito na máquina e voucher
            if (!$request->has('debit_machine')) {
                $search_text = "'debit_machine' => '1'";
                $value_text = "'debit_machine' => '0'";
                $change_content = str_replace($search_text, $value_text, $change_content);
            }
            
            if (!$request->has('voucher')) {
                $search_text = "'voucher' => '1'";
                $value_text = "'voucher' => '0'";
                $change_content = str_replace($search_text, $value_text, $change_content);
            }
