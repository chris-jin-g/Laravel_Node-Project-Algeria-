...
<script>
    ...
    var users;
    var providers;
    var lat = "{{ auth()->user()->city->lat }}";
    var longi = "{{ auth()->user()->city->longi }}";
    var ajaxMarkers = [];
    var googleMarkers = [];
    var mapIcons = {
        ...
        hold: '{{ asset("asset/img/marker-hold.png") }}'
    };

    function initMap() {
        map = new google.maps.Map(document.getElementById('map'), {
            center: {lat: parseInt(lat), lng: parseInt(longi)},
            zoom: 7
        });
        ...
        div.innerHTML = '<img src="' + mapIcons['offline'] + '"> ' + 'Motorista Indisponível';
        legend.appendChild(div);

        var div = document.createElement('div');
        div.innerHTML = '<img src="' + mapIcons['active'] + '"> ' + 'Motorista Disponivel';
        legend.appendChild(div);

        var div = document.createElement('div');
        div.innerHTML = '<img src="' + mapIcons['hold'] + '"> ' + 'Motorista em Descanso';
        legend.appendChild(div);
    ...
    function addMarkerToMap(element, index) {
        ...
        google.maps.event.addListener(marker, 'click', function() {
            window.location.href = '/fleet/provider/' +element.id;
        });
    }
</script>
